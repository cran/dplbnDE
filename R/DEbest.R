#' Discriminative parameter learning of BN by DE variant best/k/.
#'
#' Learn parameters of a Bayesian Network in a discriminative way
#' by Differential Evolution with variant best/k/
#'
#' @name DEbest
#'
  ...
  for(i in seq_len(G)){
    for(j in seq_len(NP)){
      ...
      if (mutation.pairs == 1){
        ...
        idxs <- seq_len(NP)[-j]
        R <- pop[sample(idxs, 4)]
        r0 <- best; r1 <- R[[1]]; r2 <- R[[2]]; r3 <- R[[3]]; r4 <- R[[4]]
